...
typedef DataStructure::Edge Edge;
vector<int> QQ1;
vector<Edge> QQ2;
int n;
int main() {
	...
	freopen("forest.output", "w", stdout);
	scanf("%d", &n);
	for (int i = 1; i <= n; i++) {
		int x;
		scanf("%d", &x);
	...
	DataStructure::AAATree Q2(QQ1, QQ2);
	int cnt = 0, test = 0;;
	while (true) {
		if(test == 5) {
			test++;
	...
	/*	for(int i = 1; i <= n; i++) {
			Q2.splay(Q2.reflection[i]);
			printf("%d ", Q2.reflection[i] -> linksum);
		}
		puts("");
		for(int i = 1; i <= n; i++) {
			
			Q2.splay(Q2.reflection[i]);
			printf("%d ", Q2.reflection[i] -> size);
		}
		puts("");
	*/}
	return 0;
